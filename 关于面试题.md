## 请用js实现一个监听浏览器窗口变化的函数，当浏览器窗口的宽度大于等于 600px（持续大于等于600px的话打印一次即可），请用你觉得最优的实现
```
...
## 请编写一个正则表达式，用于验证合法的 URL，比如 https://www.aliyun.com/；
## 编写一个正则表达示将 URL 中的二级域名提取出来，比如，https://www.aliyun.com/ 提取出 aliyun
```
	// 1. 验证合法的 URL
     let reg = /(http|https):\/\/[w-]+\.([\w\-_]+)+\.((com|cn|cc))/;
     let url = 'https://wwww.aliyun.com/';
     let urlArray = url.match(url);
     let isUrlValid = reg.test(url);
     console.log('url是否有效：' + isUrlValid);

    // 2. 提取二级域名
    console.log('二级域名为：' + urlArray[2]);
```
...
这段代码的执行结果？
答案：other ,不是101
解析：js中可以表示的最大整数不是2的53次方，而是1.7976931348623157e+308。
      2的53次方不是js能表示的最大整数而应该是能正确计算且不失精度的最大整数，
巩固：
var END = 1234567635;
var START = END - 1024;
var c = count = 0;
